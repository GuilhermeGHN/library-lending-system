...
public sealed class LoanReturnedProjectionHandler(IBookProjectionWriter books, ILoanProjectionWriter loans, ILogger<LoanReturnedProjectionHandler> logger) : IProjectionHandler
{
    #region Properties

    public string EventType => nameof(LoanReturnedEvent);
    ...
    {
        var message = JsonSerializer.Deserialize<LoanReturnedEvent>(payload)!;
        ...
        var loan = new LoanDocument
        {
            ...
            ReturnDate = message.ReturnDate.UtcDateTime,
        ...
        logger.LogInformation("Projected return event {EventId} for loan {LoanId}", message.EventId, message.LoanId);
    }
